...
class MySCTPNode extends NodeImplementation
{
	...
	extends Thread {
		...
		private void run_() throws java.io.IOException, InvalidAVPLengthException { if (MySCTPNode.this.serverChannel != null)
		{
		...
		while (j < arrayOfByte.length)
		{
			...
			case decoded: 
				...
				if (!bool) {
					...
					closeConnection(paramSCTPConnection); return;
				}
	...
	boolean initiateConnection(Connection paramConnection, Peer paramPeer) {
		SCTPConnection localSCTPConnection = (SCTPConnection)paramConnection;
		try {
			...
			if(paramPeer.getRealAddress() != null && paramPeer.getRealAddress().length() > 0)	{
				localInetSocketAddress = new java.net.InetSocketAddress(paramPeer.getRealAddress(), paramPeer.port());
			} else	{
				localInetSocketAddress = new java.net.InetSocketAddress(paramPeer.host(), paramPeer.port());
			}
			...
			this.logger.warn( "java.io.IOException caught while initiating connection to '" + paramPeer.host() + "'.", localIOException);
		}
		return true;
	}

	void close(Connection paramConnection, boolean paramBoolean) {
		SCTPConnection localSCTPConnection = (SCTPConnection)paramConnection;
		try {
	...
	Connection newConnection(long paramLong1, long paramLong2) {
		return new SCTPConnection(this, paramLong1, paramLong2);
	}
